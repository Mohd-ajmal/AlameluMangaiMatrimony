...
class TenthUpload extends StatefulWidget {
  const TenthUpload({Key? key}) : super(key: key);

  @override
  State<TenthUpload> createState() => _TenthUploadState();
}
...
class _TenthUploadState extends State<TenthUpload> {
  File? imageUpload;
  ...
  Widget buildGridView() {
    if (imageUpload != null) {
      ...
      return const Center(
          child: Text('Select image to upload 10th certificate'));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text("Upload 10th", style: TextStyle(color: Colors.black)),
  ...
  upload(File imageFile, {required context}) async {
    try {
      ...
      // string to uri
      var uri = Uri.parse(
          "https://alamelumangaimatrimony.com/api/v1/user-profile/user-tenth-certificate-upload/$userId");
      ...
      var multipartFile = http.MultipartFile(
          'tenth_certificate', stream, length,
