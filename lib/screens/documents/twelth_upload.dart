...
class TwelthUpload extends StatefulWidget {
  const TwelthUpload({Key? key}) : super(key: key);

  @override
  State<TwelthUpload> createState() => _TwelthUploadState();
}
...
class _TwelthUploadState extends State<TwelthUpload> {
  File? imageUpload;
  ...
  Widget buildGridView() {
    if (imageUpload != null) {
      ...
      return const Center(
          child: Text('Select image to upload 12th certificate'));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text("Upload 12th", style: TextStyle(color: Colors.black)),
  ...
  upload(File imageFile, {required context}) async {
    try {
      ...
      // string to uri
      var uri = Uri.parse(
          "https://alamelumangaimatrimony.com/api/v1/user-profile/user-twelfth-certificate-upload/$userId");
      ...
      var multipartFile = http.MultipartFile(
          'twelth_certificate', stream, length,
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              '12th certificate uploaded successfully',
            ),
          ),
        );
      } else {
